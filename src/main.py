...
from min_delta import minimize_beta, betting_on_beta, pick_best_ratio, show_stats
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        length = len(candles["BTCUSDT"])
        half = length >> 1
        print(f"Length: {length}, half: {half}")
        first_half_price_matrix: np.ndarray = np.array([[candle.close for candle in row[:half]] for row in candles.values()], dtype=float)
        second_half_price_matrix: np.ndarray = np.array([[candle.close for candle in row[half:]] for row in candles.values()], dtype=float)
